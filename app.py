...
def cidr_to_mask(prefix: int) -> str | None:
    """Convert a CIDR prefix length to a dotted IPv4 netmask."""
    if not 0 <= prefix <= 32:
        return None
    mask_number = (0xFFFFFFFF << (32 - prefix)) & 0xFFFFFFFF
    return socket.inet_ntoa(struct.pack("!I", mask_number))


def parse_linux_interfaces(ip_addr_output: str) -> list[dict]:
    """Extract interface names, IPv4 addresses and dotted masks from `ip -4 addr` output."""
    interfaces: list[dict] = []
    for heading in re.finditer(r"^\d+:\s+(\S+):", ip_addr_output, re.MULTILINE):
        name = heading.group(1)
        start = heading.end()
        nxt = re.search(r"^\d+:\s+\S+:", ip_addr_output[start:], re.MULTILINE)
        block = ip_addr_output[start:start + (nxt.start() if nxt else len(ip_addr_output))]
        inet = re.search(r"inet\s+(\d{1,3}(?:\.\d{1,3}){3})/(\d+)", block)
        if not inet:
            continue
        mask = cidr_to_mask(int(inet.group(2)))
        if mask:
            interfaces.append({"name": name, "address": inet.group(1), "mask": mask})
    return interfaces
...
def linux_interfaces() -> list[dict]:
    ...
    try:
        result = subprocess.run(["ip", "-4", "addr", "show"], capture_output=True,
                                text=True, encoding="utf-8", errors="replace", timeout=4, check=False)
        return parse_linux_interfaces(result.stdout)
    except (OSError, subprocess.SubprocessError):
        return []


def linux_wifi_ssid(primary_interface: str) -> str:
    """Best-effort SSID lookup; empty string if unavailable."""
    try:
...
class NetworkService:
    ...
    def cancel_transfer(self, identifier: str) -> bool:
        ...
        with self.active_lock:
            ...
            for entry in matches:
                entry["cancel"].set()
            # If the other side is still waiting on our decision, decline it now.
            for entry in matches:
    ...
    def _save_received_transfer(self, request: dict) -> None:
        ...
        try:
            ...
            # Clean up the partial file and tell the sender we stopped.
